...
class PL_DSAP(pl.LightningModule):
    def __init__(self, config, pretrained_ckpt=None, profiler=None, dump_dir=None):
        ...
        self.dsap_cfg = lower_config(_config['dsap'])
        self.profiler = profiler or PassThroughProfiler()
        self.n_vals_plot = max(config.TRAINER.N_VAL_PAIRS_TO_PLOT // config.TRAINER.WORLD_SIZE, 1)

        # Matcher: DSAP
        self.matcher = DSAP(config=_config['dsap'])
    ...
    def _trainval_inference(self, batch):
        ...
        with self.profiler.profile("DSAP"):
            self.matcher(batch)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.trainer.global_rank == 0 and self.global_step % self.trainer.log_every_n_steps == 0:
            ...
            # net-params
            if self.config.DSAP.MATCH_COARSE.MATCH_TYPE == 'sinkhorn':
    ...
    def test_epoch_end(self, outputs):
        ...
        if self.trainer.global_rank == 0:
            ...
            if self.dump_dir is not None:
                np.save(Path(self.dump_dir) / 'DSAP_pred_eval', dumps)
